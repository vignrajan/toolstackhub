...
// Create app/ai-prompts/seo/metadata.js with the export below
...
// ── Main page ─────────────────────────────────────────────────
export default function AiSeoPromptsPage() {
    ...
    <div className="min-h-screen bg-surface-50">
      ...
      <main className="flex-1">
        ...
        <div className="bg-gradient-to-br from-brand-600 via-brand-700 to-violet-700 text-white">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
            ...
            <p className="text-white/80 text-lg leading-relaxed max-w-3xl mb-8">
              ...
              No fluff. No generic prompts. Just prompts that actually work.
            </p>
        ...
        <div className="bg-white border-t border-b border-surface-100">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
            ...
            <p className="text-surface-500 text-center mb-10 max-w-2xl mx-auto">
              AI models like ChatGPT, Claude, and Gemini don't replace SEO expertise —
              they accelerate it. Here is what you gain with the right prompts.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {[
                { icon: '⚡', title: '10x Faster Keyword Research', desc: 'Generate 50+ keyword ideas with intent classification and content format recommendations in under 30 seconds — work that previously took hours.' },
                { icon: '📝', title: 'Publication-Ready Content Outlines', desc: 'Get detailed, SEO-structured content outlines with H2/H3 hierarchy, FAQ sections, and internal link suggestions in seconds.' },
                { icon: '🔍', title: 'On-Page Audits at Scale', desc: 'Audit meta tags, title tags, header structure, and content gaps for multiple pages simultaneously without manual review.' },
                { icon: '🏗️', title: 'Schema Markup in Minutes', desc: 'Generate complete, validated JSON-LD structured data for any page type — Article, FAQ, HowTo, Product — without memorizing the spec.' },
                { icon: '📊', title: 'Competitor Analysis Framework', desc: 'Build systematic competitor content gap analysis and keyword opportunity frameworks tailored to your specific niche and domain authority.' },
                { icon: '🔄', title: 'Content Refresh Strategy', desc: 'Identify exactly what to update in underperforming content to recover rankings — a systematic approach that consistently moves pages up.' },
              ].map(b => (
        ...
        <div className="bg-surface-100 border-t border-surface-200">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <h2 className="font-display font-bold text-xl text-surface-900 mb-5">
              More AI Prompt Libraries
            </h2>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { href: '/ai-prompts/blog',        icon: '✍️', label: 'Blog Writing Prompts'   },
                { href: '/ai-prompts/youtube',     icon: '🎬', label: 'YouTube Prompts'         },
                { href: '/ai-prompts/email',       icon: '📧', label: 'Email Marketing Prompts' },
                { href: '/ai-prompts/social-media',icon: '📱', label: 'Social Media Prompts'    },
                { href: '/ai-prompts/copywriting', icon: '💬', label: 'Copywriting Prompts'     },
                { href: '/ai-prompts/ecommerce',   icon: '🛒', label: 'eCommerce Prompts'       },
                { href: '/ai-prompts/coding',      icon: '💻', label: 'Coding Prompts'          },
                { href: '/ai-prompts/business',    icon: '🏢', label: 'Business Strategy'       },
              ].map(l => (
                <Link key={l.href} href={l.href}
                  className="flex flex-col items-center gap-2 p-4 bg-white border border-surface-200 rounded-xl hover:border-brand-200 hover:bg-brand-50 transition-colors text-center group">
                  <span className="text-2xl">{l.icon}</span>
                  <span className="text-xs font-medium text-surface-700 group-hover:text-brand-700 leading-tight">{l.label}</span>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="space-y-3" itemScope itemType="https://schema.org/FAQPage">
            {FAQS.map((faq, i) => (
              <div key={i} className="border border-surface-200 rounded-xl bg-white overflow-hidden"
                itemScope itemProp="mainEntity" itemType="https://schema.org/Question">
                <button
                  onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  className="w-full flex items-center justify-between gap-4 px-5 py-4 text-left font-medium text-surface-800 hover:bg-surface-50 transition-colors"
                  itemProp="name">
                  {faq.q}
                  <svg className={`w-4 h-4 text-surface-400 shrink-0 transition-transform duration-200 ${openFaq === i ? 'rotate-180' : ''}`}
